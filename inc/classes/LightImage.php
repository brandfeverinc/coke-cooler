...
/////////////////////////////////////////////////////////////////////////////
//
// LightImage class - represents the light_image database table
// Created on: 2016-01-13
/////////////////////////////////////////////////////////////////////////////

require_once("BaseModelPdo.php");

class LightImage extends BaseModelPdo
{
    public $Id;
    public $LightId;
    public $ImageFile;
    ...
    public function __construct(
        ...
        if (isset($this_id)
            ...
            // Retrieve this row
            $q = "SELECT * FROM light_image WHERE id = ?";
            $id = intval($this_id);
    ...
    // Get a list of all LightImage items and return as array of objects
    public function getAllLightImage($orderby = "id")
    {
        // Retrieve all Light_image
        $q = "SELECT * FROM light_image ORDER BY " . addslashes($orderby);
        $r = parent::returnResultSet($q);
        // Array to hold LightImage objects
        $arrLightImage = array();
        // Create a LightImage object for each row and add to array
        foreach ($r as $l) {
            $thisObj = new LightImage(
                $l->id,
                $l->light_id,
                $l->image_file,
                $l->sort_order
            );
            $arrLightImage[] = $thisObj;
        }
        // Return array of objects
        return $arrLightImage;
    }

    // Get a list of all LightImage items by LightId and return as array of objects
    public function getAllLightImageByLightId($light_id = null)
    {
        // Retrieve all LightImage By LightId
        $q = "SELECT * FROM light_image WHERE light_id = ? ORDER BY sort_order";
    ...
    public function delete($deleteID)
    {
        ...
        $q = "DELETE FROM light_image WHERE id = " . intval($deleteID);
        parent::updateQuery($q);
...
/////////////////////////////////////////////////////////////////////////////
    ...
    private function resetSortOrder($light_id) {
    	$q = "SELECT id, sort_order FROM light_image WHERE light_id = " . $light_id . " ORDER BY sort_order ";
        $r = parent::returnResultSet($q);
        $i = 10;
        foreach ($r as $l) {
			$q = "UPDATE light_image SET sort_order = '" . $i . "' WHERE id = '" . $l->id . "' ";
        	parent::updateQuery($q);
			$i += 10;
		}
    }
    ...
    public function sortValues($id, $direction) {
        ...
        $q = "SELECT light_id, sort_order FROM light_image WHERE id = " . $id;
        $r = parent::returnresultSet($q);
        $l = $r[0];
        ...
        if ($direction == 'U') {
            ...
            $q = "UPDATE light_image SET sort_order = " . $sortVal . " WHERE light_id = " . $light_id . " AND sort_order = " . ($sortVal-10);
        	parent::updateQuery($q);
            // Put selected item in new position
            $q = "UPDATE light_image SET sort_order = " . ($sortVal-10) . " WHERE id = " . $id;
        	parent::updateQuery($q);
        }
        else {
            // Move item that is currently in the new position to this item's position
	...
	public function getPath() {
	    return "files/lightimages/";
	}

    public function getCountLightImageByLightId($light_id)
    {
        // Retrieve all ItemImage By ItemId
        $q = "SELECT COUNT(*) As NumCount FROM light_image WHERE light_id = ?";
        $var = $light_id;
        $r = parent::returnResultSet($q, $var);
        return $r[0]->NumCount;
    }
