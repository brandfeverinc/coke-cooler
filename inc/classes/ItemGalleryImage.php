...
/////////////////////////////////////////////////////////////////////////////
//
// ItemGalleryImage class - represents the item_gallery_image database table
// Created on: 2015-12-22
/////////////////////////////////////////////////////////////////////////////

require_once("BaseModelPdo.php");

class ItemGalleryImage extends BaseModelPdo
{
    public $Id;
    public $ItemId;
    public $ItemGalleryImageUrl;
    public $SortOrder;
    ...
    public function __construct(
        $this_id = null,
        $this_item_id = null,
        $this_item_gallery_image_url = null,
        ...
        if (isset($this_id)
            ...
            foreach ($r as $l) {
                ...
                $this->ItemGalleryImageUrl = $l->item_gallery_image_url;
        ...
        // Empty constructor: set all properties to empty strings
            ...
            $this->Id = 0;
            $this->ItemId = "";
            $this->ItemGalleryImageUrl = "";
            $this->SortOrder = "";
        }
    }

    // Get a list of all ItemGalleryImage items and return as array of objects
    public function getAllItemGalleryImage($orderby = "id")
    {
        // Retrieve all Item_gallery_image
        $q = "SELECT * FROM item_gallery_image ORDER BY " . addslashes($orderby);
        $r = parent::returnResultSet($q);
        // Array to hold ItemGalleryImage objects
        $arrItemGalleryImage = array();
        // Create a ItemGalleryImage object for each row and add to array
        foreach ($r as $l) {
            $thisObj = new ItemGalleryImage(
                $l->id,
                $l->item_id,
                $l->item_gallery_image_url,
                $l->sort_order
            );
            $arrItemGalleryImage[] = $thisObj;
        }
        // Return array of objects
        return $arrItemGalleryImage;
    }

    // Get a list of all ItemGalleryImage items by ItemId and return as array of objects
    public function getAllItemGalleryImageByItemId($item_id = null)
    {
        // Retrieve all ItemGalleryImage By ItemId
        $q = "SELECT * FROM item_gallery_image WHERE item_id = ?";
    ...
    {
        $q = "UPDATE item_gallery_image SET ";
        $q .= "item_id = '" . addslashes($this->ItemId) . "', ";
        $q .= "item_gallery_image_url = '" . addslashes($this->ItemGalleryImageUrl) . "', ";
    ...
    {
        $q = "INSERT INTO item_gallery_image (";
        $q .= "item_id, ";
        $q .= "item_gallery_image_url, ";
        $q .= "sort_order";
        $q .= ") VALUES (";
        $q .= "'" . addslashes($this->ItemId) . "', ";
        $q .= "'" . addslashes($this->ItemGalleryImageUrl) . "', ";
    ...
    public function delete($deleteID)
    {
        ...
        $q = "DELETE FROM item_gallery_image WHERE id = " . intval($deleteID);
        parent::updateQuery($q);
...
/////////////////////////////////////////////////////////////////////////////
    ...
    private function resetSortOrder($item_id) {
    	$q = "SELECT id, sort_order FROM item_gallery_image WHERE item_id = " . $item_id . " ORDER BY sort_order ";
        $r = parent::returnResultSet($q);
        $i = 10;
        foreach ($r as $l) {
			$q = "UPDATE item_gallery_image SET sort_order = '" . $i . "' WHERE id = '" . $l->id . "' ";
        	parent::updateQuery($q);
			$i += 10;
		}
    }
    ...
    public function sortValues($id, $direction) {
        ...
        $q = "SELECT item_id, sort_order FROM item_gallery_image WHERE id = " . $id;
        $r = parent::returnresultSet($q);
        $l = $r[0];
        ...
        if ($direction == 'U') {
            ...
            $q = "UPDATE item_gallery_image SET sort_order = " . $sortVal . " WHERE item_id = " . $item_id . " AND sort_order = " . ($sortVal-10);
        	parent::updateQuery($q);
            // Put selected item in new position
            $q = "UPDATE item_gallery_image SET sort_order = " . ($sortVal-10) . " WHERE id = " . $id;
        	parent::updateQuery($q);
        }
        else {
            // Move item that is currently in the new position to this item's position
	...
	public function uploadFile() {
		...
		if ($success) {
		    $this->ItemGalleryImageUrl = $_FILES['image_file']['name'];
		    $this->update();
	...
	public function getPath() {
	    return "files/itemgalleryimages/";
	}


    public function GetItemGalleryImageCountByItemId($item_id)
    {
        // Retrieve all ItemImage By ItemId
        $q = "SELECT COUNT(*) As NumCount FROM item_gallery_image WHERE item_id = ?";
        $var = $item_id;
        $r = parent::returnResultSet($q, $var);
        return $r[0]->NumCount;
    }
