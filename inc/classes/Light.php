...
/////////////////////////////////////////////////////////////////////////////
//
// Light class - represents the light database table
// Created on: 2016-01-15
/////////////////////////////////////////////////////////////////////////////

require_once("BaseModelPdo.php");

class Light extends BaseModelPdo
{
    public $Id;
    public $CategoryId;
    public $OverviewText;
    ...
    // Get a list of all Light items and return as array of objects
    public function getAllLight($orderby = "id")
    {
        // Retrieve all Light
        $q = "SELECT * FROM light ORDER BY " . addslashes($orderby);
        $r = parent::returnResultSet($q);
        // Array to hold Light objects
        $arrLight = array();
        // Create a Light object for each row and add to array
        foreach ($r as $l) {
            $thisObj = new Light(
                $l->id,
                $l->category_id,
    ...
    // Get a list of all Light items by CategoryId and return as array of objects
    public function getAllLightByCategoryId($category_id = null)
    {
        // Retrieve all Light By CategoryId
        $q = "SELECT * FROM light WHERE category_id = ?";
    ...
    {
        $q = "DELETE FROM light WHERE id = " . intval($deleteID);
        parent::updateQuery($q);
    }
