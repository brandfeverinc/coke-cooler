...
array_push($aDropFields, 'item_gallery_image_url_drop');
...
if ($_POST['commit'] == "Save Item Gallery Image") {
	
	if ($id == 0) {
		// insert
        $obj = new ItemGalleryImage();
        $obj->ItemId = $_REQUEST["item"];
        $obj->create();
        
        $obj->handleFileUploads();
        $obj->handleDropFileUploads($aDropFields[0], 'ItemGalleryImageUrl');
	...
	if ($_REQUEST['mode'] == 'e') {
        ...
        $item_id = $objItemGalleryImage->ItemId;
        $item_gallery_image_url = $objItemGalleryImage->ItemGalleryImageUrl;
        $path = $objItemGalleryImage->GetPath();
	}
...
?>
	...
	<div id="content">
		...
		<div class="layout">
	        <?php
	        if ($id == 0) {
	            $bread_image_name = 'Item Gallery Image Add';
	        }
	        else {
	            $bread_image_name = 'Item Gallery Image Edit';
	        }
			...
			<div class="formstyle ">
				<form method="post" name="admin" id="admin" action="itemgalleryimage_admin.php" enctype='multipart/form-data'>
					<table class="form-table">
					    <?php 
					    if ($item_gallery_image_url != '') {
					        ?>
    						<tr>
    							<td>&nbsp;</td>
    							<td>
    							    <img src="/<?php echo $path . $item_gallery_image_url; ?>" style="width:120px;">
    							</td>
    						</tr>
    						<?php
    					}
                    	...
                    	<tr>
                    		...
                    		<td><input type="file" class="slick" size="50" maxlength="200" name="image_file" id="image_file" value="<?php echo(htmlspecialchars($item_gallery_image_url)); ?>"/>
                    		    <?php
                    		    if ($item_gallery_image_url != '') {
                    		        echo '<br ><strong>Current File:</strong> ' . $item_gallery_image_url;
                    		    }
					...
					<p class="submit"><input type="submit" name="commit" value="Save Item Gallery Image">&nbsp;&nbsp;<input type="submit" name="commit" value="Cancel"></p>
