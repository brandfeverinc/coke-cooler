...
if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == 'sort') {
    ...
    $objLightImage->SortValues(substr($_REQUEST['action'],1), substr($_REQUEST['action'],0,1));
    unset($objLightImage);
}
elseif (isset($_REQUEST['dragsort'])) {
    // handle drag sort ajax request:
    $objLightImage = new LightImage($_REQUEST['id']);
    $new_sort_order = ($_REQUEST['idx'] * 10) + 5;
    if ($new_sort_order > $objLightImage->SortOrder) {
        $new_sort_order += 10; // increasing sort order so must add another 10
    }
    $objLightImage->SortOrder = $new_sort_order;
    $objLightImage->update();
    echo "success";
        ...
        <style>
            ...
            <div class="layout center-flex">
                ...
                <div class="bigbotspace flex-container space-between">
                    ...
                    <a href="lightimage_admin.php?cat=<?php echo $_REQUEST['cat']; ?>&light=<?php echo $objLight->Id; ?>" class="button_link"><button class="">Add New Light Image</button></a>
                </div>
            </div>
