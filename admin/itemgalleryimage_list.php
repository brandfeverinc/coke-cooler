...
if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == 'sort') {
    ...
    $objItemGalleryImage->SortValues(substr($_REQUEST['action'],1), substr($_REQUEST['action'],0,1));
    unset($objItem);
}
elseif (isset($_REQUEST['dragsort'])) {
    // handle drag sort ajax request:
    $objItemGalleryImage = new ItemGalleryImage($_REQUEST['id']);
    $new_sort_order = ($_REQUEST['idx'] * 10) + 5;
    if ($new_sort_order > $objItemGalleryImage->SortOrder) {
        $new_sort_order += 10; // increasing sort order so must add another 10
    }
    $objItemGalleryImage->SortOrder = $new_sort_order;
    $objItemGalleryImage->update();
    echo "success";
        ...
        <style>
            ...
            <div class="layout center-flex">
                ...
                <div class="bigbotspace flex-container space-between">
                    ...
                    <a href="itemgalleryimage_admin.php?cat=<?php echo $objCategory->Id; ?>&item=<?php echo $_REQUEST['item']; ?>" class="button_link"><button class="">Add New Item Gallery Image</button></a>
                </div>
            </div>
            ...
            <div class="layout">
                ...
                <table class="tablestyle" id="list_table">
                    ...
                    <tbody>
                        ...
                        foreach ($oItemGalleryImage as $itemgalleryimage) {
                            echo '<tr id="img_' . $itemgalleryimage->Id . '">' . PHP_EOL;
                            echo '<td>' . $itemgalleryimage->Id . '</td>' . PHP_EOL;
                            echo '<td>';
                            if ($itemgalleryimage->ItemGalleryImageUrl != '') {
                                echo '<img src="/' . $itemgalleryimage->GetPath() . $itemgalleryimage->ItemGalleryImageUrl . '" style="width:80px;">';
                            }
                            echo '</td>' . PHP_EOL;
                            echo '<td class="mid"><img src="img/arrow-up-down.png" /></td>' . PHP_EOL;
                            echo '<td class="mid"><a href="itemgalleryimage_admin.php?cat=' . $_REQUEST['cat'] . '&item=' . $_REQUEST['item'] . '&id=' . $itemgalleryimage->Id . '"><img src="img/edit-icon.png" /></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="itemgalleryimage_delete.php?cat=' . $_REQUEST['cat'] . '&item=' . $_REQUEST['item'] . '&id=' . $itemgalleryimage->Id . '"><img src="img/delete-icon.png" /></a></td>' . PHP_EOL;
                            echo '</tr>' . PHP_EOL;
                        }
                        ?>
                    </tbody>
